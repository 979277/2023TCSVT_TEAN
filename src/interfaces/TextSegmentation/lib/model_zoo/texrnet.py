...
class semantic_refinement_head(utils.semantic_head):
    def __init__(self, 
        ...
        self.conv_sem = conv(ic_n, sem_n, 1, 1, 0, bias=False)#2,2
    ...
    def forward(self, 
        ...
        xcat = torch.cat([xref, x], axis=1)#5channel
        #import ipdb;ipdb.set_trace()

        bs, c, h, w = x.shape
        x0 = self.conv_sem(x) #2channel
        xsem = x0 + self.bias_sem.view(1, -1, 1, 1)#2channel
        #import ipdb;ipdb.set_trace()
        if self.training and not self.refinement_on:
            ...
            if self.intrain_getpred_from == 'sem':
                xsem = torch.softmax(xsem, dim=1)###2channel
                losssem['pred'] = xsem
        ...
        if self.bias_att_type in ['fixed', 'learnable', 'cossim']:
            ...
            x1 = x1/x1_sum
            # x1 [bs x sem_n x hw] now holds the key attention on each class bs*2*hw

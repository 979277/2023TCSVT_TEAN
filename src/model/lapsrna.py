...
'''class _Conv_Block(nn.Module):
    ...
    def forward(self, x):
        ...
        return output
'''
class RecurrentResidualBlock0(nn.Module):
    def __init__(self, channels):
    ...
    def forward(self, x):#,tpf
        ...
        return residual
class LapSRN(nn.Module):#nn.module
    ...
    def forward(self, x):
        ...
        for i in range(6):#block2-5
            if i<5:
                ...
                block[str(i+2)],att_inf32=self.segattention(block[str(i+2)])
                att_inf_list0.append(att_inf32)
            else :
        ...
        #HR_2x = torch.tanh((self.upb0(x))+block['8'])
        ...
        #HR_4x=torch.tanh(self.upb0(HR_2x)+block['16'])
        
        '''block['17']=self.block17(HR_4x)
        for i in range(6):#block18-23
            if i<5:
                block[str(i + 18)]= getattr(self, 'block%d' % (i + 18))(block[str(i + 17)])#,att_inf_image32
        ...
        '''
        att_inf_sum0=torch.zeros(x.size(0),1,x.size(2),x.size(3)).cuda()
        for j in range(0,len(att_inf_list0)):
            att_inf_sum0=att_inf_sum0+att_inf_list0[j]
        att_inf_avg0=att_inf_sum0/len(att_inf_list0)
        ...
        #return HR_2x,HR_4x
        return HR_2x,HR_4x,att_inf_avg0,att_inf_avg
        #return HR_2x
